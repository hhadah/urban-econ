...
quiz1 <- read.csv("/Users/hhadah/Downloads/quiz-2.csv") %>%
  select("score")
...
ggplot(quiz1_censored, aes(x = censored_score)) +
  ...
  labs(title = "Quiz 2 Scores", x = "Score", y = "Frequency") +
...
- That is, the increase in crime is caused by the event of police brutality, and not the investigations
...
- A partial test of this hypothesis is to examine police shootings of unarmed Black people in Baltimore, Chicago, and Cincinatti – three treated cities with a viral event and a PoP investigation – in the pre-investigation months but before the shootings that led to their investigation
- Is it that case that these shootings of unarmed Black people that were not followed by a PoP investigation also led to an increase in crime?
...
> We employ citizen complaint data from the Chicago Police Department and Civilian Office of Police Accountability filed between 2012-2020 to <ins>determine the extent to which BWC footage enhances</ins> **the efficacy of evidence used to formulate a conclusion of responsibility, and whether bias against complainants based on race would subsequently be reduced. **

---
## Abstract

> Accordingly, we exploit the staggered deployment of BWCs across 22 Chicago police districts over time to estimate the effect of BWCs on these outcomes. [Think a difference-in-differences]
...
- [Their findings also indicate]… a significant increase in disciplinary actions against police officers ("sustained" outcomes”) with sufficient evidence to sanction their misconduct. 
...
- Not Sustained = Dismissal of investigations due to insufficient evidence

- Sustained = Disciplinary actions against police officers

---
### Black and Hispanic individuals are more likely to have their complaints dismissed but BWCS reduce this disparity
...
---
### Officers are less likely to be disciplined for complaints from Black and Hispanic individuals and BWCs does not change this
...
> Has Black Lives Matter influenced police lethal use-of-force? 
...
The gap in lethal use-of-force between places with and without protests widens over these subsequent years and is most prominent when protests are large or frequent. This result holds for alternative specifications, estimators, police homicide datasets, and population screens; however, it does not hold if lethal use-of-force is normalized by violent crime or arrests. 
...
> Protests also influence local police agencies, which may explain the reduction. 

- Agencies with local protests become more likely to:

  1. Obtain body cameras
  2. Implement community policing programs
  3. Receive a larger operating budget from their local government
  4. Reduce the number of property crime-related arrests

- but forego some black officer employment and college education requirements.

---
## Compared to the control group, the treatment group had significantly more exposure to BLM protests
...
.pull-right[
- Method id DiD
- Treatment = census places with BLM protests
- Control = census places without BLM protests
AND
...
---
### There is a decrease in police homicides in places with protests


```{r, echo=FALSE, warning=FALSE, out.width="80%", fig.show='hold', fig.align='center'}
# Plot the image
grid.raster(readPNG("assets/Picture20.png"))
```

---
### The year of the first protest, and afterwards, areas with BLM protests experience fewer police homicides
...
We then apply our officer-level discrimination measures to various policy-relevant questions in the literature. In particular, reassigning officers across locations based on their lenience can effectively reduce the aggregate disparity in treatment. We then apply our officer-level discrimination measures to various policy-relevant questions in the literature. 
...
---
### Obvious “bunching” of tickets written as being 9, 14, 19, 24, or 29 MPH over the speed limit
...
```
  ...
  <ul>
    <li>Usually fines increase at 5 MPH increments</li>
    <li>This bunching happens more for white motorists</li>
    <li>Officers “round down” more for them</li>
  </ul>
  </div>
</div>

---
### Policies to reduce racial bias in speeding tickets discounts
...
- Option 1): Sort officers such that the <ins>least racially biased officers</ins> are in counties with the most minorities. .brand-crawfest[Reduces the gap by 11%]
- Option 2): Sort officers such that the <ins>more lenient officers</ins> are in counties with the most minorities. .brand-crawfest[Reduces the gap by 91%]
...
to_pdf(from = "~/Documents/GiT/urban-econ/slides/08-week8/01-class/racial-bias-w.html", 
       to = "~/Documents/GiT/urban-econ/slides/08-week8/01-class/racial-bias-w.pdf")
to_pdf(from = "~/Documents/GiT/urban-econ/slides/08-week8/01-class/racial-bias-w.html")
```
