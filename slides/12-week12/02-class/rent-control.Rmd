...
\begin{tikzpicture}[scale=0.7]
    ...
    \draw[thick, red] (3,0) -- (3,6) node[above] {$S_{LR}$};
    ...
    \draw[dotted] (2.2,2.2) -- (2.2,0);
    \node[below=1pt] at (2.2,0) {\scriptsize $Q_{S}^{LR}$};
    
    % Long-run quantity supplied at P_RC (now at Q = 3)
    \draw[dotted, red] (3,2.2) -- (3,0);
    \node[below=1pt] at (3,0) {\scriptsize $Q_{S}^{SR}$};
...
### While discrimination against protected groups is often illegal, it is not well enforced and not all groups are protected (e.g., LGBTQ+)
...
quiz3 <- read.csv("/Users/hhadah/Downloads/quiz-3.csv") %>%
  select("score")

# Create a censored version of the dataset
quiz3_censored <- quiz3 %>%
  mutate(censored_score = ifelse(score < 6, 6, score))

# Calculate mean and median on the original data
mean_quiz3 <- mean(quiz3$score, na.rm = TRUE)
median_quiz3 <- median(quiz3$score, na.rm = TRUE)

# Plot the histogram with censored scores and add vertical lines for mean and median
ggplot(quiz3_censored, aes(x = censored_score)) +
  geom_histogram(binwidth = 0.5, color = "black", fill = "lightblue") +
  geom_vline(xintercept = mean_quiz3, color = "red", linetype = "dashed", size = 1) +
  geom_vline(xintercept = median_quiz3, color = "blue", linetype = "dotted", size = 1) +
  annotate("text", x = mean_quiz3 - 0.2, y = 5, label = paste("Mean:", round(mean_quiz3, 2)), color = "red", angle = 90, size = 6) + 
  annotate("text", x = median_quiz3 + 0.2, y = 5, label = paste("Median:", round(median_quiz3, 2)), color = "blue", angle = 90, size = 6) + 
  labs(title = "Quiz 3 Scores", x = "Score", y = "Frequency") +
...
to_pdf(from = "~/Documents/GiT/urban-econ/slides/12-week12/02-class/rent-control.html", 
       to = "~/Documents/GiT/urban-econ/slides/12-week12/02-class/rent-control.pdf")
to_pdf(from = "~/Documents/GiT/urban-econ/slides/12-week12/02-class/rent-control.html")
```
