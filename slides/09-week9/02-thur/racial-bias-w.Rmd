...
quiz2 <- read.csv("/Users/hhadah/Downloads/quiz_2.csv") %>%
  select("score")

# Create a censored version of the dataset
quiz2_censored <- quiz2 %>%
  mutate(censored_score = ifelse(score < 8, 8, score))

# Calculate mean and median on the original data
mean_quiz2 <- mean(quiz2$score, na.rm = TRUE)
median_quiz2 <- median(quiz2$score, na.rm = TRUE)

# Plot the histogram with censored scores and add vertical lines for mean and median
ggplot(quiz2_censored, aes(x = censored_score)) +
  geom_histogram(binwidth = 0.5, color = "black", fill = "lightblue") +
  geom_vline(xintercept = mean_quiz2, color = "red", linetype = "dashed", size = 1) +
  geom_vline(xintercept = median_quiz2, color = "blue", linetype = "dotted", size = 1) +
  annotate("text", x = mean_quiz2 - 0.2, y = 10, label = paste("Mean:", round(mean_quiz2, 2)), color = "red", angle = 90, size = 3.5) + 
  annotate("text", x = median_quiz2 + 0.2, y = 10, label = paste("Median:", round(median_quiz2, 2)), color = "blue", angle = 90, size = 3.5) + 
  labs(title = "Quiz 2 Scores", x = "Score", y = "Frequency") +
  scale_x_continuous(limits = c(7.5, 11.5), 
                     breaks = c(8, 9, 10, 11),
                     labels = c("< 8", "9", "10", "11")) +
  theme_classic()
```
